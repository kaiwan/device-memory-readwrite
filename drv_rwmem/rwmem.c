...
#endif
{
  ...
  case IOCTL_RWMEMDRV_IOCGMEM: /* 'rdmem' */
    ...
    MSG("pst_rdm=%p addr: %p buf=%p len=%d flag=%d\n\n", (void *)pst_rdm,
        (void *)pst_rdm->addr, (void *)pst_rdm->buf, pst_rdm->len,
        pst_rdm->flag);
...
#else
    if (USE_IOBASE ==
        pst_rdm->flag) { // offset relative to iobase address passed
...
static int chardev_registration(void) {
  ...
  for (i = 0; i < RW_COUNT; i++) {
    ...
    res = cdev_add(
        &chardrv_devp[i].cdev,
        MKDEV(MAJOR(chardrv_dev_number), MINOR(chardrv_dev_number) + i), 1);
    if (res) {
  ...
  for (i = 0; i < RW_COUNT; i++) {
    if (!device_create(
            chardrv_class, NULL,
            MKDEV(MAJOR(chardrv_dev_number), MINOR(chardrv_dev_number) + i),
            NULL, "%s.%d", DRVNAME, i)) {
...
static int __init rwmem_init_module(void) {
  ...
  if (!iobase_start || !iobase_len) {
    pr_info("%s: Init done. IO base address NOT specified (or len invalid) as "
